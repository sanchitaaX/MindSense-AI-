...
export const FluidScene = () => {
    ...
    useEffect(() => {
        // Removed mouse listener to let it animate continuously in place
    }, []);
    ...
    useFrame((state) => {
        ...
        if (materialRef.current) {
            ...
            // Animate mouse continuously instead of following cursor
            mouse.current.x = Math.sin(t * 0.5) * 0.8;
            mouse.current.y = Math.cos(t * 0.5) * 0.8;
    ...
    // For MindSense theming, we'll override colors to fit the pink/purple theme if we want!
    const customThemeA = useMemo(() => new THREE.Color("#e91e8c"), []);
    const customThemeB = useMemo(() => new THREE.Color("#9c27b0"), []);

    return (
        <mesh>
            <icosahedronGeometry args={[1.5, 64]} />
            {/* @ts-expect-error - fluidMaterial is registered via extend */}
            <fluidMaterial
                ref={materialRef}
                key={FluidMaterial.key}
                uColorA={customThemeA}
                uColorB={customThemeB}
